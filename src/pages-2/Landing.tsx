...
const JobOpeningsPage = () => {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'table'>('table'); // Add view mode state
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-[#182230] to-[#475467] text-white py-8 container mx-auto rounded-2xl mt-6  mb-4 md:py-6">
        <div className="container mx-auto px-4">
          <h1 className="text-3xl md:text-4xl  text-[#FCFCFD] font-bold mb-6">Job Openings</h1>
            ...
            <div className="flex gap-2">
              <div className="flex-1 relative">
                ...
                <Input
                  ...
                  className="pl-10 bg-white text-black h-11"
                  value={searchTerm}
                  onChange={(e) => handleSearchChange(e.target.value)}
                  onKeyPress={(e) => e.key === 'Enter' && handleSearch()}
                />
              </div>
              <Button variant="outline" className="bg-white text-gray-700  hover:bg-white h-11 px-6" onClick={handleClearFilters}>
                Clear
              </Button>
              <Button className="bg-[#005f33] h-11 px-8" onClick={handleSearch}>
                Search
              </Button>
            </div>
          </div>
            ...
            <div className={viewMode === 'table' ? 'w-full' : 'lg:col-span-2'}>
              ...
              {jobs.length != 0 && (
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex">
                    <button
                      className={`p-2 border border-[#D0D5DD] rounded-tl-[8px] rounded-bl-[8px] ${
                        viewMode === 'table' ? 'bg-[#005f33] text-white' : 'bg-white text-[#344054]'
                      }`}
                    ...
                    <button
                      className={`p-2 border-t border-b border-r border-[#D0D5DD] rounded-tr-[8px] rounded-br-[8px] ${
                        viewMode === 'grid' ? 'bg-[#005f33] text-white' : 'bg-white text-[#344054]'
                      }`}
                  ...
                  <Card className="bg-white border border-[#E4E7EC]">
                    <CardContent className="p-12 text-center">
                      <div className="max-w-md mx-auto">
                        <h3 className="text-base font-semibold text-[#101828] mb-2">No Jobs Available</h3>
                        <p className="text-[#475467] mb-6">Make sure your profile is complete so you can start applying for new jobs.</p>
                        {/* <Button className="bg-[#0086C9] hover:bg-[#0086C9]" onClick={() => window.location.reload()}>
                        Refresh Page
                      </Button> */}
                      </div>
                        ...
                        <CardContent className="p-6">
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="text-sm text-[#005f33] font-bold mb-1">{displayJob.id}</div>
                            ...
                            <div className="flex flex-row gap-2">
                              <Button className="bg-[#005f33]" onClick={() => navigate(`/jobs/${displayJob.jobVacancyId}`)}>
                                Apply
                              </Button>
                              <button onClick={() => toggleSaveJob(job.id)} className="p-2 border border-[#005f33] rounded">
                                <Heart
                                  className={`w-5 h-5 ${savedJobs.includes(job.id) ? 'fill-red-500 text-red-500' : 'text-[#005f33]'}`}
                                />
                              </button>
                            </div>
                          </div>
                                ...
                                <td className="px-6 py-4">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      ...
                                      className="p-2 border border-[#005f33] rounded hover:bg-gray-50"
                                    >
                                      <Heart
                                        className={`w-4 h-4 ${savedJobs.includes(job.id) ? 'fill-red-500 text-red-500' : 'text-[#005f33]'}`}
                                      />
                                    </button>
                                  </div>
                                </td>
